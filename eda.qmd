...
**A NOTE** This EDA is 32 pages, FAR LONGER than what I plan to include as EDA in my final report. obviously. I will cherry pick the most interesting and necessary results from this for my final report.
...
# recoding alcohol to be in terms of days of the year, more readable, and also imputing 0s where alc_ever is 0 and alc_times_yr is na
...
nhanes_df.loc[
    (nhanes_df["alc_times_yr"].isna()) & (nhanes_df["alc_ever"] == 0),
    "alc_times_yr"
] = 0
```
...
plt.savefig('./plots/med_cap_diet.png', dpi=300, bbox_inches='tight')
plt.show()
...
Observe that dropping those individuals who didn't complete the diet data dropped NA's in other column at a faster rate, meaning those who diet data wasn't obtained for were more likely to have less data in general, dropping them again seems like a justified choice. Our dataset now contains 4137 observations. However, we observe there to be 772 missing observations in mod_ex_mins_per_week. This is high missingness, almost 20%, and this variable likely has some masking and collinearity issues, as it accounts for only moderate exercise when vigourous exercise was measured but had to be removed. It also likely has high collinearity with mins_seden_daily, as sedentary activity and physical activity are intuitively inverses of each other. Hence, sedentary activity is likely a better measure of physical activity, as it is the inverse of both moderate and vigourous activity measurements. Hence due to high missingness, inference issues, and multicollinearity, we will drop mod_ex_mins_per_week and mod_ex_per_week. We now also drop all rows containing NA values, so our EDA will be focused towards the same dataset to be used in our model. This dataset contains a total of 3004 observations.

```{python dropping mod_ex_mins_per_week}
# | echo: false

nhanes_df = nhanes_df.drop(columns = ['mod_ex_mins_per_week', 'mod_ex_per_week'])
...
Here we observe oddly that those who have lost insurance in the past 12 months have a lower median cap score, but I attribute this possibly to lower sample size.
...
Those with college degrees tend to have lower median CAP scores it seems, nad those who dropped out of high school slightly higher.
...
Interstingly, the Hepatitis B distributions appear the same, with hep_b having a greater range in their cap scores. It seems hep_b doesn't have an effect on FLD like we thought it might.
...
We see above that most individuals are concentrated with a BMI below 32.5. We also see a trend in FLD though, as BMI goes up, the proportion of people in higher FLD categories also increases. BMI will likely have a positive effect on FLD classification.
...
From this age histogram, we see most of our participants are older, with 50-80 year olds making up a majority of the sample. We also see that at young age bins, the proportion of people with high FLD categorization is low, and it increases with age.
...
Most individuals eat under 300 grams of carbs per day, and there seems to be no obvious FLD correlation here. This variable will need standardization.
...
Again, we see some individuals consuming an unhealthy amount of caffiene, some near 1800 mg. This is well over the 1200mg limit at which vomiting and other sicknesses appear. Lets investigate some high caffeine individuals.
...
to_standardize_list = ['mins_seden_daily', 'kcal', 'carb', 'sugar', 'fat', 'chol', 'water', 'salt', 'caff']

standardize_list = ['mins_seden_daily_stan', 'kcal_stan', 'carb_stan','sugar_stan', 'fat_stan', 'chol_stan', 'water_stan', 'salt_stan', 'caff_stan']
...
display(nhanes_df[to_standardize_list].agg(["mean", "std"]).T)
```

We will also create dummy variables of each catgorical covariate edu_lvl, diab, ins, race, and alc_times_yr_recat. In my dummification of alc_times_yr_recat, I will also seperate those who have never drank to those who have drank before but not in the last 12 months. This is important to seperate those who have never drank from say recovered alcoholics.
...
cols = ['edu_lvl', 'diab', 'ins', 'alc_times_yr_recat', 'race']
...
nhanes_df = nhanes_df.drop(columns = ['alc_times_yr_recat_0.0', 'race_3.0', 'edu_lvl_1.0', 'diab_0.0'])
...
# recoding alcohol so that people who have never drank are different than those who have but just havent in the last 12 months

nhanes_df['drk_but_no12'] = ((nhanes_df["alc_ever"] == 1) & (nhanes_df["alc_times_yr"] == 0)).astype(int)

nhanes_df = nhanes_df.drop(columns = 'alc_ever')
```
...
We will now examine a correlation heatmap for a possible design matrix, basically all the covariates. Furthermore, because we have over 12 variables, we are only going to keep the ones which have high correlation with each other, over 0.7 positive or negative.

```{python cov heatmap}
# | echo: false

# creating candidate deisgn matrix and making correlation heatmap for those with correlation over |0.7|

cand_list = ['cig_smoker_ever',
       'sleep_weekday', 'bmi', 'sex', 'age', 'mexican', 'hispanic', 'black', 'asian', 'other',
       'born_us', 'fam_inc_to_pov', 'hep_b',  'mins_seden_daily_stan',
       'kcal_stan', 'carb_stan', 'sugar_stan', 'fat_stan', 'chol_stan', 'water_stan', 'salt_stan', 'caff_stan', 'hs_dropout', 'hs_grad', 'aa', 'bach', 'prediabetes', 'diabetes', 'public_ins', 'private_ins', 'drk_but_no12', 'rare_drink', 'some_drink', 'often_drink']
...
We observe pefect correlation in insurance, and in some of our diet variables, particularly carbs, sugar, fat and salt. This also makes sense, as someone who eats a lot is likely to consume a lot of each macronutrient. This is expected in diet survey data however, and our analysis explanation will consider this. Our goal is to try to discern which macronutrients have an effect on FLD.

As for insurance, it seems in our removal of NA values we dropped all the individuals without insurance, Hence, we will make private insurance a binary variable over public insurance by dropping the public insurance variable.

```{python dropping public_ins}
# | echo: false

nhanes_df = nhanes_df.drop(columns = 'public_ins')
```
...
design_cand = sm.add_constant(design_cand).drop(columns = 'public_ins')
...
vif_df.to_csv('./data/vif_table.csv', index = False)
```

We observe high VIF on our dummy variables for education level. We also observe high VIF from some of our diet variables, like kcal. This confirms that some diet variables have high multicollinearity, and it may be better to keep only some of them rather than all. KCal for example is likely too high and shouldn't be considered, and is a mask for other high macronutrient covariates and bmi. Hence, in model building, I will need to be careful about which diet covariates to select.
...
```{python Appendix dropping mod_ex_mins_per_week}
# | eval: false
...
```{python Appendix dropping public_ins}
# | eval: false
